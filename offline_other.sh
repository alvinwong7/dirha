...
# Runs all of WSJ1 (LDC94S13C or Other) through nnet3 with offline decoding
...
for line in $(find ~/thesis/kaldi/egs/wsj/s5/export/corpora5/LDC94S13C -maxdepth 14 -type d); do
  ...
  for file in "$line"/*; do
    ...
    if [[ $file =~ .*\.wv1$ ]]; then
      ...
      ~/Downloads/sph2pipe_v2.5/sph2pipe $file "off_other1.wav" > /dev/null 2>&1
      ffmpeg -hide_banner -loglevel panic -y -i "off_other1.wav" -acodec pcm_s16le -ac 1 -ar 8000 "off_other2.wav"
