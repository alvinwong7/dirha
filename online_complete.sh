...
# Runs all of WSJ0 through nnet3 with online decoding
...
for line in $(find ~/thesis/kaldi/egs/wsj/s5/export/corpora5/LDC93S6A -maxdepth 14 -type d); do
  ...
  for file in "$line"/*; do
    ...
    if [[ $file =~ .*\.wv1$ ]]; then
      ...
      ~/Downloads/sph2pipe_v2.5/sph2pipe $file "on_complete1.wav" > /dev/null 2>&1
      ffmpeg -hide_banner -loglevel panic -y -i "on_complete1.wav" -acodec pcm_s16le -ac 1 -ar 8000 "on_complete2.wav"
